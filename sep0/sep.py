# -*- coding: utf-8 -*-
"""Code for handling object representation of a SEP."""
...
class SEP(object):
    ...
    def __init__(self, sep_file):
        ...
        try:
            for header_name in metadata.keys():
                ...
                if header_name != current_header:
                    raise SEPError(
                        "did not deal with "
                        "%r before having to handle %r" % (header_name, current_header),
                        sep_file.name,
                    )
        except StopIteration:
            raise SEPError("headers missing or out of order", sep_file.name)
        required = False
        try:
            while not required:
                current_header, required = next(header_order)
                ...
                raise SEPError(
                    "SEP is missing its %r" % (current_header,), sep_file.name
                )
        except StopIteration:
            pass
        # 'SEP'.
        ...
        if type_ not in self.type_values:
            raise SEPError(
                "%r is not a valid Type value" % (type_,), sep_file.name, self.number
            )
        self.type_ = type_
        # 'Status'.
        ...
        if status not in self.status_values:
            if status == "April Fool!":
                ...
                raise SEPError(
                    "%r is not a valid Status value" % (status,),
                    sep_file.name,
                    self.number,
                )
        # Special case for Active SEPs.
        if status == u"Active" and self.type_ not in ("Process", "Informational"):
            raise SEPError(
                "Only Process and Informational SEPs may " "have an Active status",
                sep_file.name,
                self.number,
            )
        # Special case for Provisional SEPs.
        if status == u"Provisional" and self.type_ != "Standards Track":
            raise SEPError(
                "Only Standards Track SEPs may " "have a Provisional status",
                sep_file.name,
                self.number,
            )
        self.status = status
        ...
        if len(authors_and_emails) < 1:
            raise SEPError("no authors found", sep_file.name, self.number)
